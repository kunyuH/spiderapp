...
def on_message_content(ws , id, option):
    ...
    # if items:
    #     for item in items:
    #         usre_name = item.find(Selector().child().type('TextView').drawingOrder(2)).text
    #         print(usre_name)
    ...
    # print('aaaaaaaaaaaa')
    # a = run_sel(lambda: ss.text(".*条评论").type("TextView").click().find())
    # return

    # 获取笔记评论量
    # 有评论  则 点击评论  点击按最新
    content_button = run_sel(lambda :ss.type("Button").desc("评论.*").find())
    if content_button:
        # 去除所有空格
